...
|---|---|---|---|---|
| 2026-07-13 | version | v0.25.1 | main | SEAL v0.25.1 (patch recette Swimlanes L29), front seul, gate Legolas PASS (614 front, aucun `src-tauri/` touché). **Fix données démo `6899b4d`** : swimlanes `iaka-demo` en barres CONTINUES pleine largeur → cause : `DEMO_TASKS` figées au 27 juin, tâches `running` (barre étendue à `Date.now()` ~16 j) + `done` sans `doneTs`. `DEMO_TASKS` const → factory `demoTasks(now=Date.now())` ancrée sur fenêtre récente ~28 min (done avec `doneTs>ts`, 1 running récente) ; `useDemoSeed` l'appelle → barres discrètes quelle que soit la date. `AgentSwimlanes` non touché. **Fix zoom+labels `2b136b1`** : (a) zoom sans effet — `Ws` clampé `[360,4200]` et tout en dérivait → échelle DÉCOUPLÉE des clamps (`scaledW=SL+R+spanMin*pxPerMin`, bornes `80/20000`) → zoom rééchelonne vraiment (largeur+ticks+barres) ; (b) labels d'heure répétés multi-jours (`HH:MM` seul) → `tickLabel` adaptatif `MM-DD HH:MM` si l'axe couvre plusieurs jours. Recette terrain validée par Stéphane. Commits `6899b4d`/`2b136b1` (+ `70ae052` note backlog L30). Doc qualité `docs/qualite/v0.25.1.md`. |
